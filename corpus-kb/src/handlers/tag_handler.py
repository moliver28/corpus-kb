...
class TagHandler:
    ...
    async def handle_add_tag(
        self, tenant_id: UUID, name: str, color: Optional[str] = None, description: Optional[str] = None
    ) -> dict[str, Any]:
        async with self._pool.acquire() as conn:
            ...
            row = await conn.fetchrow(
                ...
                str(tenant_id), name, color, description,
            )
    ...
    async def handle_tag_document(self, tenant_id: UUID, doc_id: UUID, tag: str) -> dict[str, Any]:
        async with self._pool.acquire() as conn:
            await conn.execute("SELECT set_config('app.current_tenant_id', $1, false)", str(tenant_id))
            tag_row = await conn.fetchrow("SELECT tag_id FROM tags WHERE tenant_id=$1 AND name=$2", str(tenant_id), tag)
            if not tag_row:
                await conn.execute("INSERT INTO tags (tenant_id, name) VALUES ($1, $2)", str(tenant_id), tag)
                tag_row = await conn.fetchrow("SELECT tag_id FROM tags WHERE tenant_id=$1 AND name=$2", str(tenant_id), tag)
            await conn.execute(
                "INSERT INTO document_tags (doc_id, tenant_id, tag_id) VALUES ($1, $2, $3) ON CONFLICT DO NOTHING",
                str(doc_id), str(tenant_id), str(tag_row["tag_id"]),
            )
            return {"status": "success", "doc_id": str(doc_id), "tag": tag}

    async def handle_untag_document(self, tenant_id: UUID, doc_id: UUID, tag: str) -> dict[str, Any]:
        async with self._pool.acquire() as conn:
            await conn.execute("SELECT set_config('app.current_tenant_id', $1, false)", str(tenant_id))
            await conn.execute(
                """DELETE FROM document_tags WHERE doc_id=$1 AND tenant_id=$2 AND tag_id=(
                       SELECT tag_id FROM tags WHERE tenant_id=$2 AND name=$3)""",
                str(doc_id), str(tenant_id), tag,
            )
            return {"status": "success", "doc_id": str(doc_id), "tag": tag}
    ...
    async def handle_set_metadata(
        ...
        async with self._pool.acquire() as conn:
            ...
            await conn.execute(
                ...
                key, value, str(doc_id) if doc_id else None, str(tenant_id),
            )
    ...
    async def handle_get_metadata(
        ...
        async with self._pool.acquire() as conn:
            await conn.execute("SELECT set_config('app.current_tenant_id', $1, false)", str(tenant_id))
            if key and doc_id:
                rows = await conn.fetch("SELECT key, value, doc_id FROM metadata WHERE key=$1 AND tenant_id=$2 AND doc_id=$3", key, str(tenant_id), str(doc_id))
            elif key:
                rows = await conn.fetch("SELECT key, value, doc_id FROM metadata WHERE key=$1 AND tenant_id=$2", key, str(tenant_id))
            elif doc_id:
                rows = await conn.fetch("SELECT key, value, doc_id FROM metadata WHERE tenant_id=$1 AND doc_id=$2", str(tenant_id), str(doc_id))
            else:
                rows = await conn.fetch("SELECT key, value, doc_id FROM metadata WHERE tenant_id=$1", str(tenant_id))
            return [dict(r) for r in rows]
